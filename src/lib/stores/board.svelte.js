 ...
 * @param {Tier | { label?: string; color?: string } | null | undefined} tier
...
export function getTierColor(tier, theme = 'hyv') {
	...
	// Direct color mapping from Classic to Hyv prestige
	if (color === '#ef4444') return '#FFD000';
	if (color === '#f97316') return '#A335EE';
	if (color === '#eab308') return '#0070DD';
	if (color === '#84cc16') return '#1EFF00';
	if (color === '#06b6d4') return '#CD7F32';
	if (color === '#64748b') return '#808080';

	return tier.color || '#0070DD';
}

/**
 * @param {'hyv' | 'classic'} [theme]
 * @returns {Tier[]}
 */
export function createDefaultTiers(theme = 'hyv') {
	if (theme === 'classic') {
		return [
 ...
 * @returns {Board}
 */
export function createDefaultBoard(title = 'Tier List', context = '', theme = 'hyv') {
	return {
